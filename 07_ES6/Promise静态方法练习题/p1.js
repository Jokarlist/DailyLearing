...
const proArr = new Array(10).fill(null).map((item, index) => fetchStudents(index + 1));
...
/* Promise.allSettled(proArr).then(data => {
  const stuArr = data.filter(item => item.status === "fulfilled").map(item => item.value).flat();

  console.log(stuArr);
}); */

// 获取1-10页的学生，打印最先获取到的数据，如果全部都获取失败，则打印所有的错误消息
/* Promise.any(proArr).then(data => {
  console.log(data);
}).catch(reason => {
